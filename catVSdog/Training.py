...
np.random.seed(7)
img_h, img_w = Conf.image_size[1], Conf.image_size[0]
nbatch_size = Conf.batch_size
nepochs = Conf.epochs
...
def main():
    ...
    model.add(tf.keras.layers.Conv2D(32, kernel_size=(3, 3), input_shape=(img_h, img_h, 3), activation='relu', strides=(2,2)))
    ...
    print("compile.......")
    sgd = tf.keras.optimizers.Adam(lr=Conf.learn_rate)  
    ...
    path = p + Conf.path_model
    if not os.path.exists(path):
        os.makedirs(path)
        print('Create Path \'' + path + '\' success!')
    with open(path + Conf.model_names + '.yaml', 'w') as outfile:
         outfile.write(yaml_string)
    model.save_weights(path + Conf.model_names + '.h5', save_format='h5')
